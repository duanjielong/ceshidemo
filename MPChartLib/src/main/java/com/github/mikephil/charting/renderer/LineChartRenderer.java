...
public class LineChartRenderer extends LineRadarRenderer {
        ...
        if (mDrawBitmap == null
            ...
            if (width > 0 && height > 0) {
                ...
                mDrawBitmap = new WeakReference<Bitmap>(
                    Bitmap.createBitmap(width, height, mBitmapConfig));
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            if (mLineBuffer.length
                < Math.max((entryCount) * pointsPerEntryPair, pointsPerEntryPair) * 2) {
                mLineBuffer = new float[
                    Math.max((entryCount) * pointsPerEntryPair, pointsPerEntryPair) * 4];
            }
     ...
     */
    protected void drawLinearFill(Canvas c, ILineDataSet dataSet, Transformer trans,
        XBounds bounds) {
    ...
    public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                float[] positions = trans
                    .generateTransformedValuesLine(dataSet, mAnimator.getPhaseX(), mAnimator
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                if (circleBitmap != null) {
                    c.drawBitmap(circleBitmap, mCirclesBuffer[0] - circleRadius,
                        mCirclesBuffer[1] - circleRadius, mRenderPaint);
                }
            }
        }
    }
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            MPPointD pix = mChart.getTransformer(set.getAxisDependency())
                .getPixelForValues(e.getX(), e.getY() * mAnimator
                    .getPhaseY());
        ...
        protected void fill(ILineDataSet set, boolean drawCircleHole,
            ...
            for (int i = 0; i < colorCount; i++) {
                ...
                Bitmap circleBitmap = Bitmap
                    .createBitmap((int) (circleRadius * 2.1), (int) (circleRadius * 2.1), conf);
