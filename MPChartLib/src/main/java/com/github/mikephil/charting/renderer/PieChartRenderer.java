...
public class PieChartRenderer extends DataRenderer {
    ...
    protected float calculateMinimumRadiusForSpacedSlice(
        ...
        float arcEndPointX =
            center.x + radius * (float) Math.cos((startAngle + sweepAngle) * Utils.FDEG2RAD);
        float arcEndPointY =
            center.y + radius * (float) Math.sin((startAngle + sweepAngle) * Utils.FDEG2RAD);
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            if ((Math.abs(e.getY()) > Utils.FLOAT_EPSILON)) {
                ...
                if (!mChart.needsHighlight(j)) {
                    ...
                    final float startAngleOuter =
                        rotationAngle + (angle + sliceSpaceAngleOuter / 2.f) * phaseY;
                    ...
                    if (drawInnerArc &&
                        ...
                        final float sliceSpaceAngleInner =
                            visibleAngleCount == 1 || innerRadius == 0.f ?
                                0.f :
                                sliceSpace / (Utils.FDEG2RAD * innerRadius);
                        final float startAngleInner =
                            rotationAngle + (angle + sliceSpaceAngleInner / 2.f) * phaseY;
                        ...
                        if (sweepAngleOuter >= 360.f
                            ...
                            mPathBuffer.lineTo(
                                center.x + innerRadius * (float) Math
                                    .cos(endAngleInner * Utils.FDEG2RAD),
                                center.y + innerRadius * (float) Math
                                    .sin(endAngleInner * Utils.FDEG2RAD));
                        ...
                        if (sweepAngleOuter % 360f > Utils.FLOAT_EPSILON) {
                            if (accountForSliceSpacing) {
                                ...
                                float arcEndPointX = center.x +
                                    sliceSpaceOffset * (float) Math
                                        .cos(angleMiddle * Utils.FDEG2RAD);
                                float arcEndPointY = center.y +
                                    sliceSpaceOffset * (float) Math
                                        .sin(angleMiddle * Utils.FDEG2RAD);
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    final float valueLinePart1OffsetPercentage =
                        dataSet.getValueLinePart1OffsetPercentage() / 100.f;
                    ...
                    if (drawXOutside && drawYOutside) {
                        ...
                        if (j < data.getEntryCount() && entry.getLabel() != null) {
                            drawEntryLabel(c, entry.getLabel(), labelPtx,
                                labelPty + lineHeight / 2.f);
                        }
                    } else if (drawYOutside) {

                        drawValue(c, formatter, value, entry, 0, labelPtx,
                            labelPty + lineHeight / 2.f, dataSet
                                .getValueTextColor(j));
                    }
                }
    ...
    protected void drawCenterText(Canvas c) {
        ...
        if (mChart.isDrawCenterTextEnabled() && centerText != null) {
            ...
            if (!centerText.equals(mCenterTextLastValue) || !boundingRect
                .equals(mCenterTextLastBounds)) {
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            final float startAngleShifted =
                rotationAngle + (angle + sliceSpaceAngleShifted / 2.f) * phaseY;
            ...
            if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= Utils.FLOAT_EPSILON) {
                ...
                mPathBuffer.moveTo(
                    center.x + highlightedRadius * (float) Math
                        .cos(startAngleShifted * Utils.FDEG2RAD),
                    center.y + highlightedRadius * (float) Math
                        .sin(startAngleShifted * Utils.FDEG2RAD));
