...
public class BubbleChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected float getShapeSize(float entrySize, float maxSize, float reference,
        ...
        final float factor =
            normalizeSize ? ((maxSize == 0f) ? 1f : (float) Math.sqrt(entrySize / maxSize)) :
                entrySize;
    ...
    protected void drawDataSet(Canvas c, IBubbleDataSet dataSet) {
        ...
        final float maxBubbleHeight = Math
            .abs(mViewPortHandler.contentBottom() - mViewPortHandler.contentTop());
        ...
        for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
            ...
            float shapeHalf =
                getShapeSize(entry.getSize(), dataSet.getMaxSize(), referenceSize, normalizeSize)
                    / 2f;
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    valueTextColor = Color
                        .argb(Math.round(255.f * alpha), Color.red(valueTextColor),
