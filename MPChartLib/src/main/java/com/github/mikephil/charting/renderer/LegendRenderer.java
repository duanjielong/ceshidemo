        ...
        if (!mLegend.isLegendCustom()) {
            ...
            for (int i = 0; i < data.getDataSetCount(); i++) {
                ...
                if (dataSet instanceof IBarDataSet && ((IBarDataSet) dataSet).isStacked()) {
                    ...
                    && ((ICandleDataSet) dataSet).getDecreasingColor() !=
                    ColorTemplate.COLOR_NONE) {
        ...
        switch (orientation) {
            case HORIZONTAL: {
                ...
                switch (verticalAlignment) {
                    ...
                    case CENTER:
                        posY = (mViewPortHandler.getChartHeight() - mLegend.mNeededHeight) / 2.f
                            + yoffset;
                        break;
                }
                ...
                for (int i = 0, count = entries.length; i < count; i++) {
                    ...
                    float formSize = Float.isNaN(e.formSize) ? defaultFormSize
                        : Utils.convertDpToPixel(e.formSize);

                    if (i < calculatedLabelBreakPoints.size() && calculatedLabelBreakPoints
                        .get(i)) {
                        posX = originPosX;
                    ...
                    if (!isStacked) {
                        if (drawingForm) {
                            posX +=
                                direction == Legend.LegendDirection.RIGHT_TO_LEFT ? -formToTextSpace
                                    :
                                        formToTextSpace;
                        }
                ...
                for (int i = 0; i < entries.length; i++) {
                    ...
                    if (e.label != null) {
                        ...
                        if (drawingForm && !wasStacked) {
                            posX +=
                                direction == Legend.LegendDirection.LEFT_TO_RIGHT ? formToTextSpace
                                    : -formToTextSpace;
                        } else if (wasStacked) {
                            posX = originPosX;
                        }
