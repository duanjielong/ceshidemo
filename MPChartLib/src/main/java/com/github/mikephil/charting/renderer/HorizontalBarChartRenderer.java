...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (mChart.isDrawBarShadowEnabled()) {
            ...
            for (int i = 0, count = Math
                .min((int) (Math.ceil((float) (dataSet.getEntryCount()) * phaseX)),
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                        ...
                        String formattedValue = formatter
                            .getFormattedValue(val, e, i, mViewPortHandler);
                    ...
                    while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                        ...
                        if (vals == null) {
                            ...
                            if (!mViewPortHandler
                                .isInBoundsBottom(buffer.buffer[bufferIndex + 1])) {
                                continue;
                            }
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                // calculate the correct offset depending on the draw position of the value
                                float valueTextWidth = Utils
                                    .calcTextWidth(mValuePaint, formattedValue);
                                posOffset = (drawValueAboveBar ? valueOffsetPlus
                                    : -(valueTextWidth + valueOffsetPlus));
                                negOffset = (drawValueAboveBar ? -(valueTextWidth + valueOffsetPlus)
                                    : valueOffsetPlus);
                                ...
                                float y =
                                    (buffer.buffer[bufferIndex + 1] + buffer.buffer[bufferIndex
                                        + 3]) / 2f;
