 ...
 */
public class CombinedHighlighter extends ChartHighlighter<CombinedDataProvider> implements
    IHighlighter {
    ...
    protected List<Highlight> getHighlightsAtXValue(float xVal, float x, float y) {
        ...
        for (int i = 0; i < dataObjects.size(); i++) {
            ...
            if (barHighlighter != null && dataObject instanceof BarData) {
                ...
                for (int j = 0, dataSetCount = dataObject.getDataSetCount(); j < dataSetCount;
                    j++) {
                    ...
                    List<Highlight> highs = buildHighlights(dataSet, j, xVal,
                        DataSet.Rounding.CLOSEST);
