...
public abstract class DataSet<T extends Entry> extends BaseDataSet<T> {
    ...
    /**
     * Returns a simple string representation of the DataSet with the type and the number of
     * Entries.
     */
    public String toSimpleString() {
        StringBuffer buffer = new StringBuffer();
        buffer.append(
            "DataSet, label: " + (getLabel() == null ? "" : getLabel()) + ", entries: " + mValues
                .size() +
                "\n");
        return buffer.toString();
    }
        ...
        if (closest != -1) {
            ...
            if (!Float.isNaN(closestToY)) {
                ...
                while (true) {
                    ...
                    if (Math.abs(value.getY() - closestToY) < Math
                        .abs(closestYValue - closestToY)) {
    ...
    /**
     * Determines how to round DataSet index values for {@link DataSet#getEntryIndex(float, float,
     * Rounding)} DataSet.getEntryIndex()} when an exact x-index is not found.
     */
